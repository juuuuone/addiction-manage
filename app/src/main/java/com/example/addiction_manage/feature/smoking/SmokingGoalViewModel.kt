...
class SmokingGoalViewModel @Inject constructor() : ViewModel() {
    ...
    fun setGoal(newGoal: String) {
        val currentUser = firebaseAuth.currentUser
        currentUser?.let { user -> val uid = user.uid
            val updatedGoal = SmokingGoal(newGoal)
            _goal.value = listOf(updatedGoal)
            firebaseDatabase.getReference("SmokingGoal/$uid").setValue(updatedGoal)
        }
    }
//    fun getGoal(key: String, smokingGoal: SmokingGoal) {
//        _goal.value = listOf(smokingGoal)
//        Log.d("a", "get: "+_goal.value.toString())
////        firebaseDatabase.getReference("SmokingGoal/${key}").setValue(updatedGoal)
////        firebaseDatabase.getReference("SmokingGoal").get()
////            .addOnSuccessListener { dataSnapshot ->
////                val smokingGoal = dataSnapshot.children.firstOrNull()?.let {
////                    data -> SmokingGoal(data.key!!, data.value.toString()) } ?: SmokingGoal(goal = "")
////                _goal.value = smokingGoal
////            }
//    }
...
//        firebaseDatabase.getReference("SmokingGoal/$uid").setValue(currentGoal)
//    }
//}